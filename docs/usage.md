...
Genome files provided with `--genomeinfo` must include contigs in fasta format and can optionally specify gff files (Prokka format).
Genomes fetched from NCBI are only fetched as contigs and not as gff files.
...
You will need to create a samplesheet with information about the samples you would like to analyse before running the pipeline. Use the `--input` parameter to specify its location. It must be a comma-separated file with 3 columns, and a header row as shown in the examples below:
...
Note that file paths do not have to be local, urls are also accepted.

### Multiple runs of the same sample

If you have sequenced the same sample more than once e.g. to increase sequencing depth, you can specify several rows with the same sample name.
...
#### Genomes input via "sketch index"

In addition to the `--genomeinfo` parameter and file described above, you can provide a Sourmash sketch index file pointing to
genomes available at NCBI.
This is done via the `--indexes` parameter and is a list of one or more paths (local or remote).
A collection of such files can be found at the [Sourmash prepared database page](https://sourmash.readthedocs.io/en/latest/databases.html).
(Note that a url can be provided to the `--ncbi_genome_infos` parameter so you don't need to download the file separately.)
...
Through the `--gtdb_metadata` official metadata from the [GTDB website](https://data.ace.uq.edu.au/public/gtdb/data/releases/release80/80.0/)
can be specified.

#### gtdb-tk metadata

Using the `--gtdbtk_metadata` parameter, output files from GTDB-tk can be specified.

#### checkM metadata

The `--checkm_metadata` allows you to provide either CheckM or CheckM2 output.
